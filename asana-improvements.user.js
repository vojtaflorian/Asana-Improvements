...
(function () {
  ...
  const CONFIG = {
    ...
    selectors: {
      ...
      commentExpand: ".MiniStoryContentForWebFeed-expandLink",
...
/* Task name hover expand - shows full text on hover */
${
  CONFIG.features.hoverExpandTaskNames
    ? `
/* Allow overflow on parent elements */
label.SpreadsheetTaskName {
...
.SpreadsheetRow-stickyCell {
    overflow: visible !important;
}

/* Shadow element base state - NO color override to preserve Asana theming */
.SpreadsheetTaskName-shadow {
    position: relative !important;
    cursor: pointer !important;
}

/* Hover state - shared properties */
label.SpreadsheetTaskName:hover .SpreadsheetTaskName-shadow {
...
body:not(.DesignTokenThemeSelectors-theme--darkMode) label.SpreadsheetTaskName:hover .SpreadsheetTaskName-shadow {
    background: #fff !important;
    border: 1px solid #e0e0e0 !important;
}

/* Dark mode background and border */
body.DesignTokenThemeSelectors-theme--darkMode label.SpreadsheetTaskName:hover .SpreadsheetTaskName-shadow {
...
`;
  ...
  function updateDueDates() {
    try {
      ...
      const dueDateElements = safeQuerySelectorAll(
        CONFIG.selectors.dueDateElement,
      );
  ...
  function autoExpandContent() {
    try {
      if (
        !CONFIG.features.autoExpandSubtasks &&
        !CONFIG.features.autoExpandComments
      ) {
        return;
      }
  ...
  function getCompletedTasksHiddenState() {
    try {
      const state = SafeStorage.getItem(
  ...
  function applyCompletedTasksVisibility() {
    try {
      ...
      const completedSubtasks = safeQuerySelectorAll(
        ...
        try {
          ...
          if (completedSubtasks.length > 0) {
            if (!indicator) {
              ...
              const content = label.querySelector(
                ".LabeledRowStructure-right .LabeledRowStructure-content",
              );
  ...
  function createToggleButton() {
    try {
      ...
      // Find topbar
      const topbarRightSide = safeQuerySelector(
        CONFIG.selectors.topbarRightSide,
      );
